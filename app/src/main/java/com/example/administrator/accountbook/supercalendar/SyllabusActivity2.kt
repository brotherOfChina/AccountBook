...
class SyllabusActivity2 : AppCompatActivity() {
    ...
    val financialsAdapter: AccountsAdapter by lazy {
        AccountsAdapter(R.layout.adapter_accounts, rvToDoList)
    }
    ...
    private fun showMarks() {
        async(UI) {
            ...
            for (account in accounts.await()) {
                if (account.type == "2") {
                    markData[account.create_date] = "1"
                }
            }
    ...
    fun showFinancials(date: CalendarDate) {
        ...
        async(UI) {
            val accounts = bg {
                accountDb().accountDao().getDayAccounts(dateString,"2")
            }
            ViseLog.d(accounts.await())
            financialsAdapter.data = accounts.await()
        }
    }
